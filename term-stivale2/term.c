...
void term_init(struct stivale2_struct_tag_framebuffer *fb, struct stivale2_struct_tag_memmap *mmap) {
    if (!gterm_init(&term_rows, &term_cols, fb, mmap)) {
        return;
    }
...
void term_write(uint64_t buf, uint64_t count) {
    ...
    if (!term_runtime || native) {
        ...
        while (count != 0) {
            ...
            memcpy64((uint64_t)(uintptr_t)xfer_buf, buf, chunk);
